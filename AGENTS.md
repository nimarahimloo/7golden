...
- The parallax innovations live alongside `StickyScene` in `src/components/story/`: `DepthParallax` (a three-layer depth scene — blurred atmosphere + sharp mid + foreground glass panel + travelling gold beam, all driven off one image), `MaskText` (oversized display word filled with a panning image via `background-clip: text`, used as a signature divider band) and `HorizontalScroll` (a pinned film strip where vertical scroll drives a horizontal `translateX`; `renderItem` draws each card). All three degrade to a static readable layout under `prefers-reduced-motion` — keep those fallbacks working. Their CSS (`.depth-*`, `.mask-*`, `.hscroll-*`) is in `src/styles/story.css`.
...
- `docker-compose.base44.yml` runs the Vite dev server from the mounted source on host port 3000. The `@base44/vite-plugin` sets up an `/api` dev proxy whose target is `VITE_BASE44_APP_BASE_URL`. The platform's managed value for that variable points at a local Base44 backend (`http://localhost:3001`) which is never started in this sandbox, so the compose file pins it to `https://base44.app` — without that, `src/lib/AuthContext.jsx`'s relative `/api/apps/public/...` app-state check fails with `ECONNREFUSED 127.0.0.1:3001`. Do not remove that override.
